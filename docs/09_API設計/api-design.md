...
### 5.5.1 `POST /auth/login`

S-02。`email`・`password` を受け取り、Cognito で認証する。

| 応答 | 条件 |
|---|---|
| `200` | 成功。`fs_session` を発行する |
| `401 INVALID_CREDENTIALS` | メールアドレスまたはパスワードが正しくない |

**メールアドレス未登録かパスワード不一致かは区別しない。** どちらも同じ `401 INVALID_CREDENTIALS` を返す（総当たりでの登録有無の特定を防ぐ）。

（P3-2、2026-08-16追記：`5.x` に詳細セクションがなかったため、既存の登録・エラー応答の形式に合わせて追記した）
...
（2026-08-16 解消：パスワードリセットのエンドポイントはMVPに含めない。`01_全体コンセプト` 19章#5）
