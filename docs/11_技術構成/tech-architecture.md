...
>
> **0.4の変更点**
> - **P0-2（AI推論のリージョン）を決定した。** 案B：クロスリージョン推論プロファイルを使い、データ所在は単一国を保証しない前提とする（8.4）。呼び出しリージョンはアプリ本体と同じ`ap-northeast-1`、モデルは`jp.anthropic.claude-sonnet-4-6`（頻繁な変更を前提に`api/app/ai/models.py`の定数1箇所で切り替えられる構成）
> - IAM（8.5）を、リージョンを列挙する方式からモデルIDを絞ってリージョンをワイルドカードにする方式に変更した
>
> **0.3の変更点**
...
| **AI推論のリージョン** | **案B（8.4）。グローバル推論、データ所在は単一国を保証しない** | Sonnet系は東京での In-Region 推論に非対応。**プライバシーポリシーに直結する** |
...
| 4 | ~~Sonnet 5 の推論を日本国内に留められない~~ | **解決（P0-2、2026-08-16）。** 案B（グローバル推論）を採用。データ所在は単一国を保証しない前提でプライバシーポリシーを書く（8.4） |
...
| 案 | 構成 | データの所在 | 備考 |
|---|---|---|---|
| A | `bedrock-mantle` を us-east-1 で使う | 米国内 | 所在が単一国に定まりポリシーに書きやすい。**当初の推奨だったが不採用** |
| **B** | `global.`／`jp.` などのクロスリージョン推論プロファイルを使う | **単一国を保証しない** | **採用。** どのプロファイルを使うかは頻繁に変わりうる前提 |
| C | Claude Sonnet 4.6 に下げ、日本の推論プロファイルのみを使う | 日本国内 | 国内処理が必須要件になった場合の代替。今回は不採用 |

**この判断はプライバシーポリシー（定義書19章 #1、P7-1）と一体であり、技術だけでは決められない。** ユーザー判断によりP0-2として決定した（2026-08-16）。

#### 決定：案B

**単一のリージョン・単一のモデルに固定しない。** モデルは頻繁に更新されるため、`api/app/ai/models.py` の定数1箇所を書き換えるだけで切り替えられる構成にした。

| 項目 | 値 |
|---|---|
| 呼び出しリージョン（`bedrock_region`） | **ap-northeast-1**（アプリ本体と同じ） |
| 現在のモデル（`SONNET`） | `jp.anthropic.claude-sonnet-4-6` |
| 将来切り替えうる先の例 | `global.anthropic.claude-sonnet-4-6` など |
| データの所在 | **単一国を保証しない。** プロファイルの切り替えにより処理国が変わりうる前提でプライバシーポリシーを書く |

**アプリのデータ（DynamoDB / S3 / Cognito）はap-northeast-1に留まる。** AI推論に送るのは生成に必要な入力（自由記述・対話履歴・確定済みの成果物）に限る点はA/B/Cいずれの案でも変わらない。

**IAMのARNは「どの地域からでも、指定したモデルだけ」を許可する形にした（8.5）。** クロスリージョン推論プロファイルが実際にどの地域の基盤モデルへルーティングされるかは非公開かつ変わりうるため、リージョンを列挙する方式ではなく、モデルIDを絞ってリージョンをワイルドカードにする方式に変更した（**判断の記録：** 8.5「呼ぶモデルのARNだけを許可する」という元の意図＝コスト事故の防止は、モデルIDを絞ることで満たせる。リージョンの列挙は運用上の負債になるため持たない）。
...
`Resource: "*"` にしない。**モデルを絞ることで、意図しないモデルが呼ばれてコストが跳ねる事故を防ぐ。** リージョンはワイルドカードにする（8.4）。クロスリージョン推論プロファイルの実際のルーティング先が非公開かつ変わりうるため、リージョンを列挙する方式は運用の負債になると判断した。
...
| # | リスク | 影響 | 対応 |
|---|---|---|---|
| 1 | ~~Sonnet 5 の推論を日本国内に留められない~~ | プライバシーポリシーの記述、モデル選定 | **解決（P0-2、2026-08-16）。案Bを採用（8.4）** |
...
CloudFront → API Gateway（STREAM） → Lambda（Web Adapter）
  → Bedrock mantle / jp.anthropic.claude-sonnet-4-6（ap-northeast-1）
```
...
| # | 項目 | 保留の理由 |
|---|---|---|
| ~~1~~ | ~~AI推論をどこで行うか（案A/B/C）~~ | **解消（P0-2、2026-08-16）。案B（クロスリージョン推論プロファイル、単一国を保証しない）を採用。現在のモデルは`jp.anthropic.claude-sonnet-4-6`（8.4）** |
