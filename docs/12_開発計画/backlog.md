...
| ~~**P3-10**~~ ✅ | CC | S | **S-21 アカウント登録画面のフロントエンド実装。** バックエンド（`POST /auth/register`）はP3-1で実装済みだが、画面自体が未着手のまま残っている | `04_画面設計` S-21、`06_ワイヤーフレーム`、`09_API設計` 5.5、スキル `flourish-ui` | 登録成功でS-31へ（完了画面を挟まない）。失敗時は同画面にエラー表示し入力を消さない。利用規約・プライバシーポリシーへの同意文言にP7-2で実装したページへのリンクを含む | P3-1 |
...
**P3-10が生まれた経緯：** P7-2（ポリシー・規約ページの実装）の作業中に気づいた。P3-1のタスク名は「`POST /auth/register` ＋ S-21」だが、完了メモはバックエンドのみに言及しており、`web/src/router/index.ts`にも「S-21(登録)はまだ実装されていないため、この画面への実際の遷移元はまだ無い」というコメントが残っていた。**S-21のフロントエンド（登録フォーム画面）は実際には実装されていない。** P3-2〜P3-9はいずれもS-21を経由しない前提（テスト用ルートやS-02など既存画面）で実装が進んでおり、この欠けに気づかないまま後続タスクが積み上がっていた。P7-2は暫定対応としてS-02（ログイン画面）に利用規約・プライバシーポリシーへのリンクを置いて完了条件を満たしたが、S-21自体の実装は本タスクとして切り出す。

**P3-10完了メモ（2026-08-19）：** S-21（アカウント登録画面）のフロントエンドを実装した。バックエンド（`POST /auth/register`）はP3-1で実装済みのため、本タスクは画面・APIクライアントの追加のみ。

- `web/src/views/S-21.vue`：新規。見出し「ここから先は、書いたことが残ります」・リード文・メールアドレス／パスワード入力欄・同意文言（利用規約・プライバシーポリシーへのリンク付き）・送信ボタンの構成は`mockup.html` `s21()`をそのまま踏襲した（新規に文言を考案していない）。ヘッダーは`AppHeaderSingle`（`wireframe-spec.md`「S-21登録 | ‹ 戻る | アカウント登録 | − | −」、プログレスバーなし）。レイアウトはS-11（見出し＋リード文＋下部固定CTA）とS-02（フォーム・エラー表示・入力保持）のパターンを組み合わせた
- **同意文言のリンク先は、P7-2で実装済みの`/terms-of-service`・`/privacy-policy`（公開サイト側の静的ページ）を指す。** `/app/*`の外にあるため、Vue Routerではなく通常の`<a href>`によるフルページ遷移とした（S-02のフッターリンクと同じ扱い）
- `web/src/api/auth.ts`：`register(email, password)`を追加。`POST /auth/register`を呼ぶだけで、`fs_guest`の受け渡しはサーバー側が自動で行う（`09_API設計`5.5「クライアントからゲストIDを送る必要はない」）ため、クライアント側での紐付け処理は不要
- `web/src/api/errorMessages.ts`：`EMAIL_TAKEN`・`WEAK_PASSWORD`の文言を追加した。スキル`flourish-tone`のエラー文の書き方（謝罪・感嘆符を使わない、どうすれば直るかを書く）に沿い、`EMAIL_TAKEN`は「ログインを試してみてください」という代替行動を添えた。`WEAK_PASSWORD`は流出パスワード照合・Cognitoのポリシー違反のどちらでも同じ`code`のため、原因を特定せず「別のパスワードを試してみてください」に留めた
- **成功時の遷移：** `router.push("/s-31")`。完了条件どおり完了画面を挟まない。S-16の「ありたい姿を作る」ボタンは既にP2-9時点で`/s-21`へのpushを実装済みだったため、S-16側の変更は不要だった
- **「‹ 戻る」の遷移先はS-16とした**（`screen-list.md`「戻り先: S-16」のとおり）。S-02の「トップに戻る」とは異なり、ダイアログを挟まず直接遷移する（既存の「‹ 戻るではダイアログを出さない」方針どおり）
- `web/src/router/index.ts`：`/s-21`を追加。S-16に残っていた「S-21未実装」を示唆するコメントは元々存在せず、変更不要だった
- `web/src/views/S-21.spec.ts`：成功時の`register()`呼び出しとS-31遷移、`EMAIL_TAKEN`／`WEAK_PASSWORD`それぞれでのエラー表示と入力保持、「‹ 戻る」でのS-16遷移、規約リンクの存在を確認した。`web/src/api/auth.spec.ts`・`web/src/api/errorMessages.spec.ts`にも対応するテストを追加した
- `make lint && make test`が通ることを確認済み（api 356件・tools 27件・web 342件・infra 30件、全てpass）。加えて`make dev`起動下でPlaywrightを使い、S-21の実画面をライト／ダーク両テーマで確認した。**このサンドボックス環境にはAWS認証情報がなくCognito呼び出しが失敗するため、実際の登録成功経路（S-31への遷移）はユニットテストのみで確認し、失敗経路（入力保持・エラー表示）はブラウザで実際に送信して目視確認した**（P3-2と同じ制約）
...
**P4-9完了メモ（2026-08-18）：** テーマ切替トグルを`ThemeToggle.vue`として実装し、S-41（ホーム）の`AppHeaderHub`右端に差し込んだ。バックエンドの`GET`/`PATCH /me`はP3-4で、`GET /home`が`theme_preference`を返すことはP4-8でそれぞれ実装済みだったため、本タスクの範囲はフロントエンドに閉じた。

- `web/src/components/ThemeToggle.vue`：新規。タップのたびに 自動→ライト→ダーク→自動 と循環し、現在の状態をラベル（「システムに追従」「ライト固定」「ダーク固定」）で示す（3.2）。アイコンは3.2の指定どおり、自動＝半分が塗られた円、ライト＝太陽、ダーク＝月（線画）
- **判断：「自動」アイコンの塗りつぶし。** `07_デザイン原則`7.6は「線画（アウトライン）。塗りつぶしを使わない」と定める一方、3.2は自動状態のアイコンを名指しで「半分が塗られた円」としており、文言レベルで矛盾している。3.2はこのアイコン専用の具体的な形の指定であり、7.6は一般則であるため、より具体的な3.2を優先し右半分を`fill="currentColor"`で塗った（他の2状態は7.6どおり完全な線画のまま）。停止して確認するほどの分岐（見た目の解釈が割れるだけで機能・データに影響しない）ではないと判断した
- `web/src/api/me.ts`：新規。`updateThemePreference`（`PATCH /me`）
- `web/src/stores/theme.ts`：`syncFromServer`（`GET /home`の`theme_preference`をアカウントの値として反映する。3.1「端末をまたいで一致させる」ため`PATCH`は呼ばない）と、`cycle()`のアカウント保存対応を追加した。**判断：`cycle()`は同期のまま。** 呼び出し側（トグル）にPromiseを意識させず、タップの手ごたえを即座に返すため、`setMode`で画面へ即時反映したうえで`updateThemePreference`は`await`せず`.catch`だけ付けて背景で保存する。保存に失敗しても画面の見た目はブロックしない（次回`GET /home`で再同期される）。**エラー文言は表示しない判断とした：** 09_API設計・スキルflourish-apiのどちらもテーマ保存失敗時の扱いを定めておらず、失敗してもユーザーの操作（テーマの見た目）自体は既に成立しているため、他の確定系操作（S-35の保存失敗など）と同じ「失敗を見せる」対象には含めなかった
- `web/src/views/S-41.vue`：`GET /home`成功時に`themeStore.syncFromServer(home.theme_preference)`を呼ぶよう追加。`AppHeaderHub`の`right`スロットに`ThemeToggle`を差し込んだ
- `web/src/components/AppHeaderHub.vue`：コメントをP4-9実装済みの内容に更新（P4-9の担当という将来形の記述を解消）
- `web/src/api/me.spec.ts`／`web/src/stores/theme.spec.ts`（`syncFromServer`・`cycle()`のアカウント保存・保存失敗時の挙動を追加）／`web/src/components/ThemeToggle.spec.ts`／`web/src/views/S-41.spec.ts`（新規。P4-8完了時点でテストファイルが無かったため、トグルの設置とサーバー値の反映を最小限確認する形で追加した）で確認した
- `make lint && make test`が通ることを確認済み（api 329件・web 310件・infra 30件、全てpass）。加えて`make dev`起動下で、DynamoDB Localへ直接`PROFILE`・`PURPOSE`・`AREA_PLAN`アイテムを作って認証済みセッションを用意し、Playwrightで`/s-41`をライト／ダーク両テーマで開いて実際にトグルを3回連続タップし、ラベルとアイコンが仕様どおり循環すること、サーバーログで`PATCH /api/v1/me`が3回とも200を返すこと、コンソールエラーが出ないことを確認した
...
**P5-3完了メモ（2026-08-19）：** ホームのWeekly Reflection導線（目標0個で無効、理由を添える）は、P4-8（S-41実装）の時点で先行して作られていたことを確認した。本タスクでは仕様文言との整合を1点修正し、完了条件をテストで裏付けた。

- **仕様との食い違いを1件、確認して修正した。** P4-8実装時の`web/src/views/S-41.vue`は、無効時の理由文を「目標を1つ作ると、振り返れるようになります」（読点あり）としていたが、本タスクの参照である`03_ユーザーフロー`Step 8と`04_画面設計`screen-list.md（S-41）はどちらも「目標を1つ作ると振り返れるようになります」（読点なし）を引用符付きで指定している。P4-8はこの2ドキュメントを参照範囲に含んでおらず独自に文言を作っていたため、読点を削り仕様の文言に一致させた
- `web/src/views/S-41.vue`：導線自体（`AppButton`の`disabled`バインド、無効時のみ理由文を表示、有効時はS-61へ遷移）はP4-8実装のまま変更していない。`disabled`はネイティブ`<button disabled>`で実現されており、無効時もボタン要素自体は常に描画される（消さない）ことを確認した
- `web/src/views/S-41.spec.ts`：目標0個時にボタンが存在し`disabled`属性を持つこと・理由文が表示されること・クリックしても遷移しないこと、目標1個以上時に`disabled`属性が無いこと・理由文が表示されないこと・クリックで`/s-61`へ遷移することを追加した。完了条件「無効ボタンを消さない」に対応
- `make lint && make test`が通ることを確認済み。加えて`make dev`起動下で、DynamoDB Localへ直接`PROFILE`・`PURPOSE`アイテムを作り、`AREA_PLAN`（目標1件）の有無で分けた認証済みセッションを2パターン用意し、Playwrightで`/s-41`を実際にライト／ダーク両テーマでスクリーンショット確認した（無効時：ボタンは表示されたまま操作不可・理由文あり／有効時：主ボタンとして操作可・理由文なし・タップで`/s-61`へ遷移）。コンソールエラーなし。**実際のCognito・AWS実機での疎通確認は行っていない**
...
**P6-4完了メモ（2026-08-19）：** S-01（トップページ）を、K-01/K-02と同じ静的サイトジェネレータ（P6-3）で生成する形で実装した。Vue SPA（`web/`）側には手を入れていない（S-01はアプリ画面ではなくウェブサイトのため、wireframe-spec 7.1.1）。

- **実装前に仕様の矛盾を発見し、ユーザーに確認した。** `06_ワイヤーフレーム/wireframe-spec.md`（2026-08-07更新）7.1.1はヒーローのコピーを「その『何となく』に、名前をつけるところから。」と指定していたが、`01_全体コンセプト`（2026-08-08更新、こちらが後）17.1では同じコピーを「機能の実態と合っていない」として取り下げ済みだった。ユーザーに確認し、17.1の代替案から「人生は、完成させるものじゃない。育てていくものだ。」を採用した。**`docs/06_ワイヤーフレーム/mockup.html`のS-01モックアップ（`s01()`関数）は既にこの新コピーで作られており、`wireframe-spec.md`の記述だけが更新されていなかったと分かった。** 以降のセクション文言はこのmockup.htmlを実装の一次参照とした
- `tools/generate_site.py`：`render_top_page()`を追加。ヒーロー／こんな状態、ありませんか／Flourishとは／4つの領域／できること／はじめかた／大切にしていることの7セクションに加え、「読みもの」（直近記事3件、K-01と同じ`.article-card`を再利用）とフッターを実装。`build_site()`が`index.html`を書き出すようにし、`_iter_upload_targets`・`invalidate_cloudfront`（`/`を追加）も対応させた
- **ヘッダーはK-01/K-02の「‹ 戻る」型と別に、S-01専用の`_top_header()`（サービス名＋ログインのみ、戻るなし）を新設した。** wireframe-spec 7.1.1の指定どおり
- **ログイン済みアクセス時の自動遷移（screen-list.md「例外」）は、K-01/K-02のヘッダー出し分けとは別の仕組みにした。** `GET /api/v1/me`が成功したら`location.replace("/app/s-41")`で即座に離脱する`TOP_AUTH_SCRIPT`を新設。K-01/K-02の`data-auth`属性による表示切り替えとは要件が異なる（S-01はページ自体を離れる）ため共用しなかった
- **セクション数の数え方に幅がある点は、機能的な整合性で解決した。** backlogは「7セクション」、`screen-list.md`は「読みもの」を含めて8項目、mockup.htmlは9ブロック（7セクション＋読みもの＋フッター）。矛盾として止めるほどではなく、K-01への導線（screen-list.mdの遷移先要件）を満たすために「読みもの」とフッターも実装した
- **フッターの「利用規約／プライバシーポリシー」はリンクにしなかった。** P7-1（文面）・P7-2（ページ実装）が未着手のため、mockup.html同様プレーンテキストとして表示し、存在しないページへのリンクを作らない判断をした
- タイポグラフィは`07_デザイン原則`4.2の型（画面見出し20px、本文15px、補足13px、ラベル11px）に統一した。mockup.htmlのヒーロー見出し27pxなど独自サイズは採用していない（デザイン原則にLP向けの例外規定がなく、wireframe-spec 8章の「本文15pxのタイプスケールに全画面を揃える」という決定と整合させるため）
- `tools/site_assets/site.css`：`.lp-*`クラス群を追加。レイアウト幅は本文系セクションを`--layout-width-article`（640px）、「読みもの」の記事グリッドを`--layout-width-public`（960px）とした。下部固定CTAは`position: fixed`（wireframe-spec「スクロール位置によらず常に出す」を厳密に満たすため、`sticky`ではなく`fixed`を選んだ）
...
**P7-2完了メモ（2026-08-19）：** P7-1の成果物（`privacy-policy.md`・`terms-of-service.md`）を、K-01/K-02と同じ静的サイトジェネレータ（P6-3）でHTML化し、公開サイト・アプリ内の両方から到達できるようにした。

- **着手前に仕様のズレを発見し、ユーザーに確認した。** 当初はアカウント登録画面（S-21）の同意文言にリンクを置く想定だったが、S-21のフロントエンドが未実装だと判明した（P3-1のタスク名は「`POST /auth/register` ＋ S-21」だが、完了メモはバックエンドのみに言及し、`web/src/router/index.ts`にも「S-21(登録)はまだ実装されていない」というコメントが残っていた）。ユーザーに確認し、**既存のS-02（ログイン画面）にリンクを置く**方針で解消した。**S-21のフロントエンド自体が未実装である点は、本タスクの範囲外の別タスクとして残る**（P3-1は完了マークがついているが、画面部分は積み残しになっている）
- `tools/generate_site.py`：`_render_legal_body`を新規実装。この2文書に十分な範囲（見出しh2・テーブル・箇条書き・段落・`**太字**`・`` `コード` ``）だけを扱う専用の最小限のMarkdown変換で、汎用ライブラリは導入していない（対象文書が2つに限られ、今後も大きく増える見込みがないため）。冒頭のバージョン管理メタデータ（1つ目の`---`まで）は内部向けのため出力せず、1つ目と2つ目の`---`の間にある注記（運営者情報が未確定である旨）は、内部タスクID部分（例：「（P7-9 本番デプロイ）」）を取り除いたうえで表示する。**判断の記録：** 法務文書のソースに内部の開発管理用語（タスクID）が混在しており、機械的にそのまま転記すると公開ページに漏れてしまうため、パターンマッチで除去した。実質的な開示内容（運営者情報は追って追記する）自体は変更していない
- `render_legal_page`：`/privacy-policy`・`/terms-of-service`にHTML（K-02と同じ`.article`レイアウト）を生成。`build_site`・`_iter_upload_targets`（S3キーは記事と同じ拡張子なし規約）・`invalidate_cloudfront`・`render_sitemap`に組み込んだ
- `tools/site_assets/site.css`：`.article__body`に`h2`・`table`・`ul`・`code`・`.legal-notice`のスタイルを追加（既存の`--font-size-section`等のトークンを再利用、新規トークンは追加していない）
- **公開サイト側の到達経路：** S-01フッターの「利用規約／プライバシーポリシー」（P6-4時点ではP7-1/P7-2未完了のためプレーンテキストだった）を実リンクに変更した
- **アプリ内側の到達経路：** `web/src/views/S-02.vue`のフッターに「利用規約／プライバシーポリシー」リンクを追加した。`/privacy-policy`・`/terms-of-service`は`/app/*`の外（CloudFrontのdefaultBehavior経由で公開サイト用S3バケットへ）にあるため、Vue Routerではなく通常の`<a href>`によるフルページ遷移とした
- リンクの見た目は`flourish-ui`スキルの「Primaryを使ってよい場所」表の「リンクテキスト」に従い`--primary`色＋下線とした
