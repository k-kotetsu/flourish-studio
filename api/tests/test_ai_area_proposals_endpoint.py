"""`POST /ai/area-proposals`。09_API設計6章、10_AIプロンプト設計4.6の検証・認証・
`PURPOSE_REQUIRED`(409)・レート制限・非同期ジョブ登録を確認する。

実際のBedrock呼び出し・SQS送信は行わない。`send_job_message`をフェイクに差し替える
(test_ai_purpose_proposals_endpoint.pyと同じ手法)。
"""
...
def test_returns_401_without_session_cookie() -> None:
    ...
    response = client.post("/api/v1/ai/area-proposals", json=_request_body())
...
def test_returns_422_when_choices_are_missing_a_question(
    ...
    monkeypatch.setattr("app.api.v1.ai_area_proposals.send_job_message", fake_send)
    client = _client_with_logged_in_user()
    body = _request_body()
    body["choices"] = _VALID_CHOICES[:2]

    response = client.post("/api/v1/ai/area-proposals", json=body)
...
def test_returns_429_when_rate_limited(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.v1.ai_area_proposals.rate_limit.check_and_increment_user",
        _raise_rate_limited,
    )
...
def test_returns_202_and_queues_a_job(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert job_item["kind"] == "AREA_PROPOSALS"
...
def test_idempotency_key_reuses_the_same_job(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = client.post("/api/v1/ai/area-proposals", json=_request_body(), headers=headers)
    second = client.post("/api/v1/ai/area-proposals", json=_request_body(), headers=headers)
